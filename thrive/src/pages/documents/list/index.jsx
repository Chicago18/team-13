...
export class DocumentListPage extends React.Component {
  ...
  render() {
    ...
    for (var i = 0; i < this.documentList.length; i++) {
        documents.push(
          <DocumentCard title=<Link to={"/documents/"+this.documentList[i].id}>{this.documentList[i].title}</Link> description={this.documentList[i].description.substring(0,240)+"..."} />)
    };
